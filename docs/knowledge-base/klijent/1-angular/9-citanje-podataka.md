...
2. Definition of resource; httpResource<T>(() => url) declared as a field of the page component [code + text]
3. The URL function: it re-runs when a signal it reads changes; constant URL versus URL built from a signal; the project's blog detail, whose URL reads the route input, as the example [code + text]
