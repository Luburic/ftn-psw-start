...
Outline:
1. Anchor: two pages need the same list; the dependency container recalled from the server segment, which is the strong callback here [text]
...
4. A service holding a signal; a page reading it through protected readonly svc = inject(Svc) and svc.items() in the template [code + text]
5. Integration: the tour list page and the my-tours page both injecting the Tours service, read from the project [code + analysis]
