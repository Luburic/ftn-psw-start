...
3. The mechanics of one command method in a service: await the post, reload the affected resource, return the result [code + text]
4. The page pattern: pending signal, error signal, try / catch / finally [code + text]
...
Out of scope: forms, optimistic updates, RxJS operators, the body of serverMessage.
