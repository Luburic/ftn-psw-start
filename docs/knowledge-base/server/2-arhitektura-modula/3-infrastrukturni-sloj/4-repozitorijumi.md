Aplikacioni sloj je za svaki agregat deklarisao dva interfejsa. Repozitorijum agregata obećava da vraća i prima cele agregate, a repozitorijum za čitanje da vraća DTO strukture spremne za prikaz. Infrastrukturni sloj ta obećanja ispunjava pomoću kontekstne klase. Ovde razmatramo tri pitanja:
1. Kako kontekst vraća agregat u celini?
2. Kako saznaje šta je metoda agregata promenila?
3. Kako se definišu repozitorijumi koji podržavaju operacije upita?

## Repozitorijum agregata

Repozitorijum agregata implementira interfejs aplikacionog sloja tako što svaku metodu prevodi u rad sa `DbSet` svojstvom korena. Sledeći kod prikazuje repozitorijum ankete:
...
Maper podrazumevano čita samo tabelu na koju se upit odnosi. **Učitavanje povezanih objekata** (engl. *eager loading*) je način učitavanja pri kom upit unapred navodi koje povezane redove treba pročitati zajedno sa glavnim, tako da se rezultat sastavi u jednom obraćanju bazi. U EFC se to navodi metodom `Include`:
...
- Poziv `Include` imenuje kolekciju koju treba popuniti. EFC upit proširuje spajanjem (`JOIN`) tabela `Surveys` i `Questions` po stranom ključu `SurveyId`. Jedan `SELECT` tako vraća red ankete ponovljen uz svaki red pitanja, a od tih redova EFC sastavlja jednu anketu sa popunjenom kolekcijom.
- Vrednosni objekti ne traže `Include`, jer nemaju sopstvenu tabelu. Opcije su deo reda pitanja i stižu sa njim.

Agregat se po definiciji učitava u celini, pa bi svaki upit nad anketom morao da ponovi isti `Include`, a upit koji ga zaboravi vraća agregat koji ne ume da brani svoja pravila. EFC zato dopušta da se učitavanje kolekcije proglasi obaveznim u konfiguraciji, jednom za sve upite:
...
Svaki praćeni objekat ima jedno od četiri stanja:
- Objekat vraćen iz upita je *nepromenjen*.
- Objekat čije se svojstvo ili polje razlikuje od pročitanog je *izmenjen*.
- Objekat koji je predat kontekstu pozivom `Add` ili se dodao u kolekciju koju referencira praćeni objekat je *dodat*.
- Objekat koji je uklonjen iz praćene kolekcije, a ne sme da postoji van nje, je *obrisan*.

Posmatrajmo šta kontekst radi kada komanda nad anketom u pripremi ukloni jedno pitanje, doda drugo, objavi anketu i sačuva izmene kroz repozitorijum:
...
- Metoda `UpdateAsync` samo poziva `SaveChangesAsync`, jer kontekst već prati anketu i sva njena pitanja, pa ne treba da mu se kaže šta je izmenjeno. Poziv `SaveChangesAsync` poredi svaki praćeni objekat sa zapamćenim stanjem i za svaki izmenjen, dodat ili obrisan objekat sastavlja po jednu naredbu. Za dati primer:
   - Anketa je *izmenjena*, jer je `Publish` promenio status, i dobija `UPDATE`.
   - Uklonjeno pitanje je *obrisano*, jer veza ka anketi ne dopušta pitanje bez ankete, i dobija `DELETE`.
   - Novo pitanje je *dodato* i dobija `INSERT` sa identifikatorom koji mu je dodelio konstruktor.
   - Preostala pitanja su *nepromenjena* i ne dobijaju ništa.
...
Upiti aplikacionog sloja ne menjaju stanje, pa im praćenje ne treba, a ne treba im ni agregat, jer vraćaju DTO strukture. Repozitorijum za čitanje zato podatke iz baze projektuje pravo u DTO strukturu, što sledeći primer ilustruje:
...
Repozitorijum agregata i repozitorijum za čitanje dele isti kontekst i iste tabele, a razlikuju se u tome šta od konteksta traže. Prvi traži praćen agregat koji će komanda menjati, a drugi nepraćene podatke koje će upit vratiti.